<!--#include file="../common.asp"-->
<!--#include file="../DBhelper.asp"-->
<!-- METADATA TYPE="typelib" FILE="C:\Program Files\Common Files\System\ado\msado15.dll" -->
<object runat="server" progid="ADODB.Command" id="Command" viewastext="viewastext"></object>
<%
	Call Init()
	' request : guildId,guildMemberId,itemHash,usableGuildLevel,days
	' response : itemHash,expireDate
	
	Dim i,j,k
	Dim sph,rs,retString,blsResult : blsResult = False

	' preparing request parameters
	Dim guildId,guildMemberId,itemHash,usableGuildLevel,days
	If UBound(parameters)<4 Then Call Error("not enough parameter")
	guildId = Parameters(0)
	guildMemberId = Parameters(1)
	itemHash = Parameters(2)
	usableGuildLevel = Parameters(3)
	days = Parameters(4)
	
	Dim expireDate
	
	' call Stored procedure
	Set sph = new SPHelper
	with sph
		.DEBUG = False
		Set .cmd = Command
		.ConnStr = guildDBconnectionString
		.SPName = "dbo.public_AddGuildShopProduct"
		Call .InitCommand()
		Call .AppendParam("@guildId",adInteger,adParamInput,,guildId)
		Call .AppendParam("@guildMemberId",adInteger,adParamInput,,guildMemberId)
		Call .AppendParam("@itemHash",adInteger,adParamInputOutput,,itemHash)
		Call .AppendParam("@usableGuildLevel",adInteger,adParamInput,,usableGuildLevel)
		Call .AppendParam("@days",adInteger,adParamInput,,days)
		Call .AppendParam("@expireDate",adDBTimeStamp,adParamOutput,,null)
		blsResult = .ExecNoRecords()
	End with

	If blsResult=False Then
		if sph.frk_n4ErrorCode>0 Then
			Call Error(sph.frk_n4ErrorCode)
		Else
			Call Error(sph.frk_strErrorText)
		End If
	Else
		itemHash = sph.GetParamValue("@itemHash")
		expireDate = sph.GetParamValue("@expireDate")
	End If
	Set sph = Nothing
	
	retString = retString & guildId & SEPARATOR & itemHash & SEPARATOR & FormatDt(expireDate,"SQL_TM")

	Call Ok(retString)
%>