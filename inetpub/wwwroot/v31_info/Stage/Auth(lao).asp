...
<%
	...
	'			,Items(subseparated: itemHash,isOnBank,bagNumber,positionNumber,stackedCount,itemSerial,instance)
	'			,skills(subseparated: skillGroupHash,skillLevel)
	'			,quickslots(subseparated: hash,position,isSkill,instance)
	...
	'			,petItems(subseparated: petSerial,bagNumber,positionNumber,itemHash,stackedCount,itemSerial,instance)
	...
	' basic : classNumber,stageLevel,stageExp,pvpLevel,pvpExp,warLevel,warExp,gameMoney,bankMoney,skillPoint,instantStateFlags,createDate
	Response.Write SEPARATOR & rs(0) & SEPARATOR & rs(1) &_
			...
			SEPARATOR & FormatDt(rs(11),"SQL_TM")
	...
	set rs = rs.NextRecordset
	If Not(rs.Eof) Then
		Response.Write SEPARATOR & rs(0) & SEPARATOR & rs(1) & SEPARATOR & rs(2) & SEPARATOR & FormatDt(rs(3),"SQL_TM")
	Else
		Response.Write SEPARATOR & 0 & SEPARATOR & 0 & SEPARATOR & 0 & SEPARATOR & FormatDt("1900-01-01","SQL_TM")
	End If
	...
	Else
		Response.Write SEPARATOR & 1500 & SEPARATOR & 0 & SEPARATOR & 0 & SEPARATOR & 0
	End If
	...
	' items : itemHash,isOnBank,bagNumber,positionNumber,stackedCount,itemSerial,instance
	i = 0
	Response.Write SEPARATOR
	Do Until rs2.Eof
		If i>0 Then Response.Write SUBSEPARATOR
		Response.Write rs2(0) & SUBSEPARATOR & CInt(rs2(1)) & SUBSEPARATOR & rs2(2) &_
				SUBSEPARATOR & rs2(3) & SUBSEPARATOR & rs2(4) & SUBSEPARATOR & rs2(5) & SUBSEPARATOR & rs2(6)
		rs2.MoveNext : i=i+1
	Loop
	rs2.Close
	...
	' quickslots : hash,position,isSkill,instance
	i = 0
	Response.Write SEPARATOR
	set rs = rs.NextRecordset
	Do Until rs.Eof
		...
		Response.Write rs(0) & SUBSEPARATOR & rs(1) &_
				SUBSEPARATOR & CInt(rs(2)) & SUBSEPARATOR & rs(3)
		rs.MoveNext : i=i+1
	Loop
	...
	' pets : petSerial,petName,petLevel,petExp,fullValue,isRare,rareProbability,fullRateSum,dtSum,petHash
	i = 0
	Response.Write SEPARATOR
	set rs = rs.NextRecordset
	Do Until rs.Eof
	...
	' petItems : petSerial,bagNumber,positionNumber,itemHash,stackedCount,itemSerial,instance
	i = 0
	Response.Write SEPARATOR
	set rs = rs.NextRecordset
	Do Until rs.Eof
	...
	' petTraining : petSerial,itemHash,stackedCount,instance,expFactor,startTime,endTime
	i = 0
	Response.Write SEPARATOR
	set rs = rs.NextRecordset
	Do Until rs.Eof
		...
		Response.Write rs(0) & SUBSEPARATOR & rs(1) & SUBSEPARATOR & rs(2) &_
				...
				SUBSEPARATOR & FormatDt(rs(5),"SQL_TM") &_
