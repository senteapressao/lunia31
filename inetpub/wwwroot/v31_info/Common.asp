...
<%
	' -- common codes ------------------------------------------------------
	Dim characterDBconnectionString,stageDBconnectionString,guildDBconnectionString,gateDBconnectionString,cashDBconnectionString,rankDBconnectionString
	characterDBconnectionString	= GetConnStr("DESKTOP-S29QIQ5\SQLEXPRESS,3061","v3_character_info","sa","algoraSaturn2203@")
	stageDBconnectionString	= GetConnStr("DESKTOP-S29QIQ5\SQLEXPRESS,3061","v4_stage_info","sa","algoraSaturn2203@")
	guildDBconnectionString	= GetConnStr("DESKTOP-S29QIQ5\SQLEXPRESS,3061","v3_guild_info","sa","algoraSaturn2203@")
	gateDBconnectionString	= GetConnStr("DESKTOP-S29QIQ5\SQLEXPRESS,3061","v3_gate_info","sa","algoraSaturn2203@")
	cashDBconnectionString	= GetConnStr("DESKTOP-S29QIQ5\SQLEXPRESS,3061","d-shop","sa","algoraSaturn2203@")
	rankDBconnectionString	= GetConnStr("DESKTOP-S29QIQ5\SQLEXPRESS,3061","crmdb","sa","algoraSaturn2203@")
	' predefined constant
	const ServerGroupName = "TW-R"
	const ServerGroupCode = 126
	const LoginWSDL = "http://DESKTOP-S29QIQ5\SQLEXPRESS,3061/v4/a.txt"
	
	Function Logger2(CustomData)
		Dim rowData, arrPath, fileName, reciver, dir, tfile, count, data
		...
		if  fileName <> "Charac1terUpdate" and fileName <> "ListSqua1reStatus" and fileName <> "Pin1g.Check" then
			dir = Server.MapPath("..\..\logs") & "\[" & category & "]"&fileName&".txt"
			set tfile=CreateObject("Scripting.FileSystemObject").OpenTextFile(dir, 8, true)
			tfile.WriteLine("CustomData : " & CustomData)
			tfile.close		
		end if
    End Function
	...
	Function getConvAsciiIgnore(value)
		Dim strV,i,s,intChr,ichr1,ichr2,strS
		 s = LenB(value) : strV = "" :
		 For i = 1 to s
			intChr = AscB(MidB(value,i,2))
			if intChr > 128 then
				strV = strV & Chr(63) 'strV = strV & Chr(strS)
			Else
				strV = strV & Chr(intChr)
			End If
		 Next
		 getConvAsciiIgnore = strV
	End Function
